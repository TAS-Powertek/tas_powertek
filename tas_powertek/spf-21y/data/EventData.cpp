...
/* static */ EventData EventData::fromByteStream(std::string_view data) {
  if (data.size() != sizeof(EventData)) {
    throw std::runtime_error(fmt::format(
        "Cannot deserialize data of size {} into EventData of size {}",
        data.size(), sizeof(EventData)));
  }

  EventData result;
  std::memcpy(&result, data.data(), sizeof(EventData));
  result.bigEndian();
  return result;
}
}  // namespace tas_powertek::spf21y